...
public class BxpSensorPayloadActivity extends MkGw4BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBind = ActivityBxpSensorPayloadMkgw4Binding.inflate(getLayoutInflater());
        setContentView(mBind.getRoot());
        EventBus.getDefault().register(this);
        showSyncingProgressDialog();
        MokoSupport.getInstance().sendOrder(OrderTaskAssembler.getMkSensorPayload());
            ...
            if (MokoConstants.ACTION_ORDER_RESULT.equals(action)) {
                ...
                if (orderCHAR == OrderCHAR.CHAR_PARAMS) {
                    if (value.length >= 4) {
                        ...
                        if (flag == 0x01) {
                            // write
                            if (configKeyEnum == ParamsKeyEnum.KEY_MK_SENSOR_PAYLOAD) {
                                ToastUtils.showToast(this, (value[4] & 0xff) == 1 ? "Setup succeed" : "Setup failed");
                            }
                        } else if (flag == 0x00) {
                            // read
                            if (configKeyEnum == ParamsKeyEnum.KEY_MK_SENSOR_PAYLOAD && length == 2) {
                                ...
                                mBind.cbTH.setChecked((data >> 11 & 0x01) == 1);
                            }
                        }
                    }
                }
    ...
    private void onSave() {
        ...
        int payload = (mBind.cbRssi.isChecked() ? 1 : 0) | (mBind.cbTimestamp.isChecked() ? 1 << 1 : 0) |
                ...
                (mBind.cbRawDataRes.isChecked() ? 1 << 10 : 0) | (mBind.cbTH.isChecked() ? 1 << 11 : 0);
        MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setMkSensorPayload(payload));
    }
